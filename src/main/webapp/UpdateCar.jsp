...
<div class="form-container">
    ...
    <form  action="update-car?carDetailsId=${carDetails.carId}" method="post"  id="form" enctype="multipart/form-data"  >
        <div class="form-group">
            <input type="hidden" name="carId" value="${carDetails.carId}">

            <label for="name">Name:</label>
            <input type="text" id="name" name="name" value="${carDetails.getName()}" >
        </div>
        <div class="form-group">
            <label for="registrationNumber">Registration Number:</label>
            <input type="text" id="registrationNumber" name="registrationNumber" value="${carDetails.getRegistrationNumber()}">
        </div>
        <div class="form-group">
            <label for="policyNumber">Policy Number:</label>
            <input type="text" id="policyNumber" name="policyNumber" value="${carDetails.getInsurancePolicyNumber()}">
        </div>
        <div class="form-group">
            <label for="fuelType">Fuel Type:</label>
            <select id="fuelType" name="fuelType" >
                <option value="Petrol" ${carDetails.getFuelType() == 'Petrol' ? 'selected' : ''}>Petrol</option>
                <option value="Diesel" ${carDetails.getFuelType()  == 'Diesel' ? 'selected' : ''}>Diesel</option>
                <option value="Electric" ${carDetails.getFuelType()  == 'Electric' ? 'selected' : ''}>Electric</option>
            </select>
        </div>
<%--        <div>--%>
...
<%--        </div>--%>
        <div>
            ...
            <br><br>
            <img id="preview" src="${carDetails.getImage()}" alt="Selected Image" style="max-width: 400px; max-height: 400px;">
        ...
        <div class="form-group">
            ...
            <select id="transmissionType" name="transmissionType" value="${carDetails.getTransmissionType()}"  >
                ...
                <option value="Automatic" ${carDetails.getTransmissionType() == 'Automatic' ? 'selected' : ''}>Automatic</option>
                </select>
            </select>
        </div>
        <div class="form-group">
            <label for="carColor">Color:</label>
            <input type="text" id="carColor" name="carColor" value="${carDetails.getColor()}" >
        </div>
        <div class="form-group">
            <label for="kmTravelled">Kilometers Travelled:</label>
            <input type="text" id="kmTravelled" name="kmTravelled" value="${carDetails.getKmTravelled()}">
        </div>
        <div class="form-group">
            <label for="ratePerDay">Rate Per Day:</label>
            <input type="text" id="ratePerDay" name="ratePerDay" value="${carDetails.getChargePerDay()}">
        </div>
        <div class="form-group">
            <label for="modelNumber">Model Number:</label>
            <input type="text" id="modelNumber" name="modelNumber"value="${carDetails.getModel()}" >
        </div>
        <div class="form-group">
            <label for="noOfSeats">No of Seats:</label>
            <input type="number" id="noOfSeats" name="noOfSeats" value="${carDetails.getNoOfSeats()}">
        </div>
        <div class="form-group">
